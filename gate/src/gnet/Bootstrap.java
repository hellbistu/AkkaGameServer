...
import gnet.config.ConfigManager;
import gnet.config.GateConfig;
import gnet.handler.DispatchHandler;
import gnet.handler.KeepAliveHandler;
import gnet.handler.PacketDecoder;
import gnet.handler.PacketEncoder;
...
public class Bootstrap {
    ...
    public static void main(String[] args) {
        ...
        ActorSystem gateActorSystem = ActorSystem.create(String.format("gate-%d",config.getGateId()));

        ActorRef actorRef = gateActorSystem.actorOf(Props.create(GateRoot.class));
        actorRef.tell("init", ActorRef.noSender()); //通知gate actor system初始化。目前异步初始化就行
